...
/****** Object:  Job [_DBA_Backup]    Script Date: 8/6/2016 3:46:42 PM ******/
...
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Database Maintenance]    Script Date: 8/6/2016 3:46:42 PM ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Cleanup Phase 1 - Gather Data]    Script Date: 8/6/2016 3:46:42 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Cleanup Phase 1 - Gather Data', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Cleanup Phase 2 - Delete Old Database Backup Files]    Script Date: 8/6/2016 3:46:42 PM ******/
...
$CheckArchiveBit_SQL = Invoke-SqlCmd -Query "SELECT CAST(ISNULL(DBA.dbo.fn_GetOptVal(''Server'', ''CheckArchiveBitOnDelete''), 0) AS BIT)" -Database "DBA" -ServerInstance $Instance 
$CheckArchiveBit = $false 

If($CheckArchiveBit_SQL -eq "1")
{
...
ForEach ($Row in ($FilesToDelete))
{
	...
	If((Test-Path -LiteralPath $File -PathType Leaf) -eq $false)
	{
		
		#Could not locate file, marking as deleted.
	...
	else
	{
		...
		#File Exists, begin tests.
		$ArchiveAttribute = [system.IO.FileAttributes]::Archive 
		If((Get-ItemProperty -LiteralPath $File).Attributes -band $ArchiveAttribute)
		{
			#ArchiveBit is true, so file has NOT been backed up yet.
			$HasBeenBackedUp = $false 	
		}
		else
		{
			#ArchiveBit is false, so file HAS been backed up.
			$HasBeenBackedUp = $true  
		}
		
		#Delete the file IF
		#	- Check Archive Bit is false
		# 	- OR Check Archive Bit is true AND Has been Backed Up is true.
		if((($CheckArchiveBit -eq $true) -and ($HasBeenBackedUp -eq $true)) -or ($CheckArchiveBit -eq $false))
		{
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Cleanup Phase 3 - Remove old database Object script files]    Script Date: 8/6/2016 3:46:42 PM ******/
...
ForEach ($Row in ($BackupObjectsToDelete))
{
	...
	If ((Test-Path -Path (''Microsoft.Powershell.Core\FileSystem::'' + $File) -PathType Leaf) -eq $true )
	{
	
		#Remove the File...
		Remove-Item -Path (''Microsoft.Powershell.Core\FileSystem::'' + $File) -ErrorAction SilentlyContinue 
...
Invoke-SqlCmd -Query "UPDATE Data_ObjectBackupHistory SET DateRemoved = GETDATE() WHERE DateRemoved IS NULL" -Database "DBA" -ServerInstance $Instance', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 1 - Create Folder Structure]    Script Date: 8/6/2016 3:46:42 PM ******/
...
ForEach ($Row in ($Folders))
{
	...
	If ((Test-Path -Path $Folder -PathType Container) -eq $false)
	{
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 2 - Database]    Script Date: 8/6/2016 3:46:42 PM ******/
...
EXEC DBA.dbo.spBackupDatabase @DbName, @BackupType, @IsDebug', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 3a - SQL Agent Jobs]    Script Date: 8/6/2016 3:46:42 PM ******/
...
If ((Test-Path -Path (''Microsoft.Powershell.Core\FileSystem::'' + $RootFolder) -PathType Container) -eq $false)
{
...
If ((Test-Path -Path (''Microsoft.Powershell.Core\FileSystem::'' + $RootFolder + "!Agent Job List.csv") -PathType leaf) -eq $true)
{
	Remove-Item -Path (''Microsoft.Powershell.Core\FileSystem::'' + $RootFolder + "!Agent Job List.csv") -ErrorAction SilentlyContinue 
}

#Writeout the master list.
$MasterList | Export-Csv (''Microsoft.Powershell.Core\FileSystem::'' + $RootFolder + "!Agent Job List.csv") -NoTypeInformation -NoClobber -Encoding ASCII', 
		@database_name=N'master', 
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 3b - Backup Logins]    Script Date: 8/6/2016 3:46:42 PM ******/
...
ForEach ($Login In $FilesToWrite)
{
	...
	#Create the folder?
	If ((Test-Path -Path (''Microsoft.Powershell.Core\FileSystem::'' + $FolderPath) -PathType container) -eq $false)
	{
		New-Item -Path (''Microsoft.Powershell.Core\FileSystem::'' + $FolderPath) -ItemType container
	}
	
	#Delete the file if it already exists, we are overwriting.
	If ((Test-Path -Path (''Microsoft.Powershell.Core\FileSystem::'' + $FolderPath + "\" + $LoginFileName) -PathType leaf) -eq $true)
	{
		Remove-Item -Path (''Microsoft.Powershell.Core\FileSystem::'' + $FolderPath + "\" + $LoginFileName) -ErrorAction SilentlyContinue 
	}
	
	#Write the file contents.
	$LoginScript | Out-File (''Microsoft.Powershell.Core\FileSystem::'' + $FolderPath + "\" + $LoginFileName) -Encoding ASCII -NoClobber -ErrorAction SilentlyContinue  
...
#Create the folder?
If ((Test-Path -Path (''Microsoft.Powershell.Core\FileSystem::'' + $TrackingFolder) -PathType container) -eq $false)
{
	New-Item -Path (''Microsoft.Powershell.Core\FileSystem::'' + $TrackingFolder) -ItemType container
}

$TrackingFolder = $TrackingFolder + "!Login Script.csv"

#Delete the file if it already exists, we are overwriting.
If ((Test-Path -Path (''Microsoft.Powershell.Core\FileSystem::'' + $TrackingFolder) -PathType leaf) -eq $true)
{
	Remove-Item -Path (''Microsoft.Powershell.Core\FileSystem::'' + $TrackingFolder) -ErrorAction SilentlyContinue 
}

$FilesToWrite | Export-Csv -Path (''Microsoft.Powershell.Core\FileSystem::'' + $TrackingFolder) -NoClobber -NoTypeInformation -Encoding "ASCII"
', 
		@database_name=N'master', 
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 3c - Roles]    Script Date: 8/6/2016 3:46:42 PM ******/
...
If ((Test-Path -Path (''Microsoft.Powershell.Core\FileSystem::'' + $RootFolder) -PathType Container) -eq $false)
{
	New-Item -Path (''Microsoft.Powershell.Core\FileSystem::'' + $RootFolder) -ItemType directory
}
...
ForEach ($Database In $SMO.databases)
{
	...
	ForEach ($Role IN $Database.roles)
	{
		...
		if ($Role.IsFixedRole -eq $false)
		{
			...
			If ((Test-Path -Path (''Microsoft.Powershell.Core\FileSystem::'' + $Folder) -PathType Container) -eq $false)
			{
				New-Item -Path (''Microsoft.Powershell.Core\FileSystem::'' + $Folder) -ItemType directory
			}
...
If ((Test-Path -Path (''Microsoft.Powershell.Core\FileSystem::'' + $RootFolder + "!Role List.csv") -PathType leaf) -eq $true)
{
	Remove-Item -Path (''Microsoft.Powershell.Core\FileSystem::'' + $RootFolder + "!Role List.csv") -ErrorAction SilentlyContinue 
}

#Writeout the master list.
$MasterList | Export-Csv (''Microsoft.Powershell.Core\FileSystem::'' + $RootFolder + "!Role List.csv") -NoTypeInformation -NoClobber -Encoding ASCII', 
		@database_name=N'master', 
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 3d - Schemas]    Script Date: 8/6/2016 3:46:42 PM ******/
...
If ((Test-Path -Path (''Microsoft.Powershell.Core\FileSystem::'' + $RootFolder + "!Schema List.csv") -PathType leaf) -eq $true)
{
	Remove-Item -Path (''Microsoft.Powershell.Core\FileSystem::'' + $RootFolder + "!Schema List.csv") -ErrorAction SilentlyContinue 
}

#Writeout the master list.
$MasterList | Export-Csv (''Microsoft.Powershell.Core\FileSystem::'' + $RootFolder + "!Schema List.csv") -NoTypeInformation -NoClobber -Encoding ASCII', 
		@database_name=N'master', 
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 3e - SSIS Packages]    Script Date: 8/6/2016 3:46:42 PM ******/
...
ForEach ($SSISPackage In $FilesToWrite)
{
	...
	#Delete the file if it already exists, we are overwriting.
	If ((Test-Path -Path (''Microsoft.Powershell.Core\FileSystem::'' + $FolderPath + "\" + $SSISPackageFileName) -PathType leaf) -eq $true)
	{
		Remove-Item -Path (''Microsoft.Powershell.Core\FileSystem::'' + $FolderPath + "\" + $SSISPackageFileName) -ErrorAction SilentlyContinue 
	}
	
	#Write the file contents.
	$SSISPackageScript | Out-File (''Microsoft.Powershell.Core\FileSystem::'' + $FolderPath + "\" + $SSISPackageFileName) -Encoding ASCII -NoClobber -ErrorAction SilentlyContinue 
...
$List | Export-Csv -Path (''Microsoft.Powershell.Core\FileSystem::'' + $TrackingFolder) -NoClobber -NoTypeInformation -Encoding "ASCII"
', 
		@database_name=N'master', 
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 3f - Database Build Scripts]    Script Date: 8/6/2016 3:46:42 PM ******/
...
ForEach ($Database In ($SMO.databases | Where-Object {$_.IsSystemObject -eq $false}))
{
	...
	Invoke-SqlCmd -Query $Query -ServerInstance $Instance -Database "DBA"
		...
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 3g - Linked Servers]    Script Date: 8/6/2016 3:46:42 PM ******/
...
ForEach ($LinkedServer IN $SMO.LinkedServers)
{
	...
	Invoke-SqlCmd -Query $Query -ServerInstance $Instance -Database "DBA"
		...
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 3h - SSAS]    Script Date: 8/6/2016 3:46:42 PM ******/
...
ForEach ($SSASDB in $AMO.Databases)
{
	...
	If ((Test-Path -Path (''Microsoft.Powershell.Core\FileSystem::'' + $DBFolder) -PathType Container) -eq $false)
	{
		New-Item -Path (''Microsoft.Powershell.Core\FileSystem::'' + $DBFolder) -ItemType directory
	}
	...
	If ((Test-Path -Path (''Microsoft.Powershell.Core\FileSystem::'' + $BackupFile) -PathType Container) -eq $true)
	{
		Remove-Item -Path (''Microsoft.Powershell.Core\FileSystem::'' + $BackupFile) -ErrorAction SilentlyContinue 
	}
...
', 
		...
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 4 - Update Agent Job History]    Script Date: 8/6/2016 3:46:42 PM ******/
...
EXEC DBA.dbo.sp_agntJobHist', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Logging Cleanup]    Script Date: 8/6/2016 3:46:42 PM ******/

...
/****** Object:  Job [_DBA_Backup_Hourly]    Script Date: 02/08/2017 1:18:36 PM ******/
...
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Database Maintenance]    Script Date: 02/08/2017 1:18:37 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'_DBA_Backup_Hourly', 
		...
		@notify_email_operator_name=N'DBA Operator', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Phase 1 - Create Folder Structure]    Script Date: 02/08/2017 1:18:37 PM ******/
...
ForEach ($Row in ($Folders))
{
	...
	If ((Test-Path -Path $Folder -PathType Container) -eq $false)
	{
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Phase 2 - Backup Database]    Script Date: 02/08/2017 1:18:37 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Phase 2 - Backup Database', 
		...
		@command=N'
--Backup all databases using the LOG method.
...
EXEC DBA.dbo.spBackupDatabase NULL, ''L'', 0;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Phase 3 - Gather Data]    Script Date: 02/08/2017 1:18:37 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Phase 3 - Gather Data', 
		...
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'
--Read the trace file, looking for auto-growth events.

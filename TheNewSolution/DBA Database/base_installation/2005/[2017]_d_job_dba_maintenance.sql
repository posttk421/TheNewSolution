...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Index Defragmentation', 
		...
		@command=N'DECLARE	@DefragmentThreshold FLOAT 
SET @DefragmentThreshold = 5.0
DECLARE	@RebuildThreshold FLOAT 
SET @RebuildThreshold = 30.0
DECLARE	@IsExecuteSQL BIT 
SET @IsExecuteSQL = 1
DECLARE	@DbName VARCHAR(250)
SET @DbName = NULL
DECLARE	@DefragDelayText VARCHAR(20) 
SET @DefragDelayText = ''12:00:00 AM''
DECLARE	@Script VARCHAR(MAX) 


EXEC [DBA].[dbo].[spDefragmentIndexes] 
   @DefragmentThreshold
  ,@RebuildThreshold
  ,@IsExecuteSQL
  ,@DbName

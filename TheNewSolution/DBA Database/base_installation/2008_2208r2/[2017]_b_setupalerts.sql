...
This script is designed to setup Alerts for various error conditions.

It removes some legacy items found on some of the servers and then proceeds
to create the desired alerts.  It ensures the operator exits, creates 
some new categories and then creates alerts for each of the necessary types.

This script is also used to update existing systems so it removes alerts by the same name.  

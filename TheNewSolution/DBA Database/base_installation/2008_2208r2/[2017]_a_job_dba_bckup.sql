...
EXEC msdb.dbo.sp_delete_job @job_id=N'208e6831-edbb-46bd-808d-f22eec09b120', @delete_unused_schedule=1
GO
...
ForEach ($Row in ($FilesToDelete))
{
	...
	#File must exist
	# AND (file has been backed up and check archive bit is true
	# 	OR check archive bit is false)
	If (((Test-Path -Path $File -PathType Leaf) -eq $true) -and ((($CheckArchiveBit -eq $true) -and ($HasBeenBackedUp -eq $true)) -or ($CheckArchiveBit -eq $false)))
	{
	
		#File exists
		#Check Archive bit is false
		#OR if check archive bit is true, then file has been backed up.
	...
	elseif ((Test-Path -Path $File -PathType Leaf) -eq $false)
	{
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Cleanup Phase 3 - Remove old database Object script files]    Script Date: 02/08/2017 12:00:00 AM ******/
...
', 
		...
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 3c - Roles]    Script Date: 02/08/2017 12:00:00 AM ******/
...
$MasterList | Export-Csv ($RootFolder + "!Role List.csv") -NoTypeInformation -NoClobber -Encoding ASCII', 
		...
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 3d - Schemas]    Script Date: 02/08/2017 12:00:00 AM ******/
...
$MasterList | Export-Csv ($RootFolder + "!Schema List.csv") -NoTypeInformation -NoClobber -Encoding ASCII', 
		...
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 3e - SSIS Packages]    Script Date: 02/08/2017 12:00:00 AM ******/
...
ForEach ($Database In ($SMO.databases | Where-Object {$_.IsSystemObject -eq $false}))
{
	...
	Invoke-SqlCmd -Query $Query -ServerInstance $Instance -Database "DBA"
		...
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 3g - Linked Servers]    Script Date: 02/08/2017 12:00:00 AM ******/
...
ForEach ($LinkedServer IN $SMO.LinkedServers)
{
	...
	Invoke-SqlCmd -Query $Query -ServerInstance $Instance -Database "DBA"
		...
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 3h - SSAS]    Script Date: 02/08/2017 12:00:00 AM ******/

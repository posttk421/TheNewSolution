...
/****** Object:  Job [_DBA_Backup]    Script Date: 02/08/2017 16:27:18 ******/
IF  EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'_DBA_Backup')
...
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Database Maintenance]    Script Date: 02/08/2017 16:27:18 ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Cleanup Phase 1 - Gather Data]    Script Date: 02/08/2017 16:27:18 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Cleanup Phase 1 - Gather Data', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Cleanup Phase 2 - Delete Old Database Backup Files]    Script Date: 02/08/2017 16:27:18 ******/
...
ForEach ($Row in ($FilesToDelete))
{
	...
	If ((Test-Path -Path $File -PathType Leaf) -eq $true )
	{
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Cleanup Phase 3 - Remove old database Object script files]    Script Date: 02/08/2017 16:27:19 ******/
...
Invoke-SqlCmd -Query "UPDATE Data_ObjectBackupHistory SET DateRemoved = GETDATE() WHERE DateRemoved IS NULL" -Database "DBA" -ServerInstance $Instance', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 1 - Create Folder Structure]    Script Date: 02/08/2017 16:27:19 ******/
...
ForEach ($Row in ($Folders))
{
	...
	If ((Test-Path -Path $Folder -PathType Container) -eq $false)
	{
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 2 - Database]    Script Date: 02/08/2017 16:27:19 ******/
...
EXEC DBA.dbo.spBackupDatabase @DbName, @BackupType, @IsDebug', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 3a - SQL Agent Jobs]    Script Date: 02/08/2017 16:27:19 ******/
...
$MasterList | Export-Csv ($RootFolder + "!Agent Job List.csv") -NoTypeInformation -NoClobber -Encoding ASCII', 
		...
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 3b - Backup Logins]    Script Date: 02/08/2017 16:27:19 ******/
...
', 
		...
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 3c - Roles]    Script Date: 02/08/2017 16:27:19 ******/
...
$MasterList | Export-Csv ($RootFolder + "!Role List.csv") -NoTypeInformation -NoClobber -Encoding ASCII', 
		...
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 3d - Schemas]    Script Date: 02/08/2017 16:27:19 ******/
...
$MasterList | Export-Csv ($RootFolder + "!Schema List.csv") -NoTypeInformation -NoClobber -Encoding ASCII', 
		...
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 3e - SSIS Packages]    Script Date: 02/08/2017 16:27:19 ******/
...
', 
		...
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup Phase 3f - Database Build Scripts]    Script Date: 02/08/2017 16:27:19 ******/
...
ForEach ($Database In ($SMO.databases | Where-Object {$_.IsSystemObject -eq $false}))
{
	...
	#Create the ancillary items for each table (including Foreign Keys).
	ForEach ($Table In $Database.Tables)
	{
	
		ForEach ($Index In $Table.Indexes)
		{
			$Scripter.Script($Index)
		}
		...
		ForEach ($Statistics In $Table.Statistics)
		{
			$Scripter.Script($Statistics)
		}
		
		ForEach ($ForeignKey In $Table.ForeignKeys)
		{
			$Scripter.Script($ForeignKey)
		}
		
	}
	
	ForEach ($View In ($Database.Views | Where-Object {$_.Schema -ne "sys"}))
	{
	...
	ForEach ($Proc in ($Database.StoredProcedures | Where-Object {$_.IsSystemObject -eq $false -and $_.IsEncrypted -eq $false -and $_.Schema -ne "sys"}))
	{	
		$Scripter.Script($Proc)		
	}
	
	ForEach ($Function In ($Database.UserDefinedFunctions | Where-Object {$_.IsSystemObject -eq $false -and $_.IsEncrypted -eq $false -and $_.Schema -ne "sys"}))
	{
		$Scripter.Script($Function)
	}
	
	ForEach ($DataType In ($Database.UserDefinedDataTypes | Where-Object {$_.IsSystemObject -eq $false -and $_.IsEncrypted -eq $false -and $_.Schema -ne "sys"}))
	{
		$Scripter.Script($DataType)
	}
	
	ForEach ($Aggregate In ($Database.UserDefinedAggregates | Where-Object {$_.IsSystemObject -eq $false -and $_.IsEncrypted -eq $false -and $_.Schema -ne "sys"}))
	{
		$Scripter.Script($Aggregate)
	}
	
	ForEach ($TableType In ($Database.UserDefinedTableTypes | Where-Object {$_.IsSystemObject -eq $false -and $_.IsEncrypted -eq $false -and $_.Schema -ne "sys"}))
	{
		$Scripter.Script($TableType)
	}
	
	ForEach ($Types In ($Database.UserDefinedTypes | Where-Object {$_.IsSystemObject -eq $false -and $_.IsEncrypted -eq $false -and $_.Schema -ne "sys"}))
	{
		$Scripter.Script($Types)
	}
	
	ForEach ($Trigger In ($Database.Triggers | Where-Object {$_.IsSystemObject -eq $false -and $_.IsEncrypted -eq $false -and $_.Schema -ne "sys"}))
	{
		$Scripter.Script($Trigger)
	}
	...
	Invoke-SqlCmd -Query $Query -ServerInstance $Instance -Database "DBA"
		...
		@flags=32
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Logging Cleanup]    Script Date: 02/08/2017 16:27:19 ******/

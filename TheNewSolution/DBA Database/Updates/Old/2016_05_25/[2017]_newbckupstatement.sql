...
/****** Object:  UserDefinedFunction [dbo].[fn_CreateBckupStatemnt]    Script Date: 02/08/2017 09:13:19 ******/
SET ANSI_NULLS ON
GO
...
DECLARE @TimeStamp VARCHAR(200) = CAST(REPLACE(REPLACE(CONVERT(VARCHAR(20), GETDATE(), 120), ':', '_'), '-', '_') AS NVARCHAR(100))
...
BEGIN
	SET @BckupSizeGB = (	SELECT TOP 1 ISNULL(CompressedSizeGB, BckupSizeGB) 
							...
							WHERE DatabaseName = @DbName ORDER BY DateGathered DESC
							)

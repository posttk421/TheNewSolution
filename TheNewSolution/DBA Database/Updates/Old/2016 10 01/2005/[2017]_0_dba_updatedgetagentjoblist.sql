...
/****** Object:  StoredProcedure [dbo].[sp_GetAgtJoblist]    Script Date: 02/08/2017 12:00:00 AM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[sp_GetAgtJoblist]
AS
...
SET @LastGathered = ISNULL((SELECT MAX(DateLastRun) FROM Data_AgentJobList WHERE DateLastRun IS NOT NULL), DATEADD(wk, -6, GETDATE()))
...
SELECT @@SERVERNAME
	...
	, A.NextRunTime
FROM @AgentJobList A
WHERE NOT(A.Job_ID IN (SELECT D.Job_ID FROM Data_AgentJobList D))
